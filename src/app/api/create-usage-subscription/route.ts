...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create usage-based subscription using Dodo Payments API
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          error: 'Failed to create usage-based subscription', 
          details: errorData,
          status: response.status,
    ...
    console.error('Usage-based subscription creation error:', error)
    return NextResponse.json(
